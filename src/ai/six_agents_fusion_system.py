    ...
    async def _get_agent_decision(self, agent_id: str, agent_info: Dict, market_data: Dict) -> Dict:
        ...
        try:
            ...
            if level == AgentLevel.META_COMMANDER.value:
                # 元学习AI决策 - 最复杂的决策逻辑
                signal = np.tanh(np.random.normal(0, 0.3))
                confidence = 0.8 + np.random.normal(0, 0.1)
                reasoning = "元学习决策"
                
            elif level == AgentLevel.INTEGRATION_COORDINATOR.value:
                # 集成学习协调AI决策
                signal = np.tanh(np.random.normal(0, 0.25))
                confidence = 0.75 + np.random.normal(0, 0.1)
                reasoning = "集成协调决策"
                
            elif level == AgentLevel.REINFORCEMENT_EXECUTOR.value:
                # 强化学习执行AI决策
                signal = np.tanh(np.random.normal(0, 0.2))
                confidence = 0.7 + np.random.normal(0, 0.1)
                reasoning = "强化学习决策"
                
            elif level == AgentLevel.TIMESERIES_PROPHET.value:
                # 时序深度学习AI决策
                signal = np.tanh(np.random.normal(0, 0.15))
                confidence = 0.65 + np.random.normal(0, 0.1)
                reasoning = "时序预测决策"
                
            elif level == AgentLevel.TRANSFER_ADAPTER.value:
                # 迁移学习适配AI决策
                signal = np.tanh(np.random.normal(0, 0.1))
                confidence = 0.6 + np.random.normal(0, 0.1)
                reasoning = "迁移学习决策"
                
            else:  # EXPERT_GUARDIAN
                # 专家系统守护AI决策
                signal = np.tanh(np.random.normal(0, 0.05))
